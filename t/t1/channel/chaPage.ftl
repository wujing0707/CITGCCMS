		...
		$.post("../content/queryContentWithPage.action", 
			...
			function(data) {
				...
				$.each(obj,function(n,value) {
			         	 var trs = "";  
			         	 trs +=  "<li><a href=" + value.contentId + '.shtml' + " target='blank'>" + value.title + "</a><span class=\"floatR\">" + value.releaseTime + "</span></li>";
			             tbody += trs;     
			           });  
				$("#li").html(tbody);  
...
<#if i = 0>
<div class="leftMenuTitle"><div class="txt"  onclick="window.location.href='../${channelTargetFilePath!}${parent.path!}${suffixes!}';">
 ${parent.name!}
 </div></div>
<ul>
<#else> 
	<#if channel.path=parent.path>
		<li id="${parent.path}" parentId="${parent.channelId}" class="current" onclick="window.location.href='../${channelTargetFilePath!}${parent.path!}${suffixes!}';"> ${parent.name}</li>
	<#else> 
		<li id="${parent.path}" parentId="${parent.channelId}"  onclick="window.location.href='../${channelTargetFilePath!}${parent.path!}${suffixes!}';"> ${parent.name}</li>
	</#if>
</#if>  
<#assign i = i + 1/>  
